...
You DO a light codebase discovery to identify probable files DEVELOPER will touch, which saves search time downstream. Use Grep / Glob, or `node "${CLAUDE_PLUGIN_ROOT}/scripts/ts-symbols.mjs" sym <name>` to locate a named symbol's file. Do NOT reach for `LSP`: it is not available to a harness agent, and checking costs a turn (see `.claude/rules/lsp-usage.md`). Light discovery only, no deep reading.
...
**But do not set it on every ticket that merely WRITES an already-constrained column.** One constraint is one risk, and it is carried by the ticket that introduces it — not again by each consumer. A feature adding one constrained column otherwise comes back with it set on the data layer, the form, the import and the merge paths alike, most of them touching no `supabase/` file, which puts nearly every reviewer on the expensive model and makes review the largest line in the run. When an earlier ticket in the same plan already introduces and reviews the constraint, and this one writes the same column through the same validated helper, leave it `false` and let the dependency carry the risk.
