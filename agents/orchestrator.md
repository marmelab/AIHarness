...
**Prefer FOREGROUND, and treat the dispatch acknowledgement as the truth about what happens next.** Ask for `run_in_background: false` on every pipeline dispatch: a call that blocks and returns the child's contract line inline is the simplest thing to reason about, and `force-foreground-orchestrator-dispatch` denies an explicitly backgrounded one. Some runtimes do not expose the parameter to a nested subagent at all; that is accepted, not an error, and there is nothing for you to fix when it happens.

**If the acknowledgement says you will be notified, the notification IS the completion mechanism.** An `Async agent launched ... agentId: <id>` reply means dispatched, not finished, and it also means you WILL be re-woken when that child completes. So:

- **NEVER re-dispatch on an async acknowledgement.** Ending your turn after one is correct behaviour, not a dead end. Re-dispatching "in case the first one is lost" puts two opus planners on the same `TICKETS_DIR`, overwriting each other's tickets; `block-duplicate-dispatch` refuses it while the first is still in flight.
...
- it was `REJECTED:` -> follow the Stage 2 retry path. The flag is correctly absent.
- it was `APPROVED` -> the hook could not parse it. `<session_dir>/hooks.log` has the `[record-review-verdict]` line for that stop with `role=`, `task=` and `verdict=`, which says whether the miss was the verdict, the ticket or the identity. Report it as a harness defect in your handoff, and re-run the SAME reviewer dispatch once with `WRITE_VERDICT_FLAG: yes` added to the prompt so the reviewer records the flag itself on this runtime.
- there is no contract line at all -> the review did not complete. Re-dispatch it.
...
On a demo-smoke FAIL or `status: "failed"`, take the SAME action as a `BLOCKED:` feature review, with the findings replaced by the failing spec output: dispatch ONE `developer` with the SIMPLE template (`CHANGE_REQUEST` = the failing output verbatim, `BRANCH_NAME: <SESSION_SHORT_ID>/simple`, `WORKTREE_PATH: <WORKTREE_BASE>/simple`, `run_in_background: false`), then merge it into `session/<SESSION_SHORT_ID>` with a SIMPLE-mode merger carrying `STAGE: a-only`. **Stop there: the merger's stop re-runs the suite by itself.** Do NOT re-run feature-review to trigger it. Re-run the review only when the fix touched application code in a way that plausibly invalidates the review's own findings; a change confined to `e2e/` never does.

**Count fix rounds PER FAILURE, not globally.** `e2e-result.json` carries a `failureSignature`: 2 fix rounds for a given signature, and a NEW signature resets the count for that new failure. A global budget is spent by whichever bugs happen to surface first, so a suite with three defects ships red on the third without one attempt at it. Hard cap: **6 e2e fix rounds total** for the request, whatever the signatures, as a runaway stop. An empty `failureSignature` means the output was not parseable; count those against the global cap only. Past either bound, report the remaining failure in the handoff and proceed, never wedge the pipeline. The feature-review rounds keep their own 2-round bound.

A skip is fine (deferred to a human `make test-e2e-ci`, the headless target; `make test-e2e` opens the Playwright UI and needs a display). `#technical-harness` runs feature-smoke before its stop and reports the result; it does not promote.
...
- ❌ Create, touch, edit, or delete any file under `<session_dir>/reviews` or `<session_dir>/breaker`: those are the review-verdict and dispatch-debounce guards. Fabricating an approval flag or clearing a dispatch marker bypasses review; `bash-guard` blocks it. The `record-review-verdict` hook writes the verdict flags; you only ever read these dirs.
