...
// Namespaces the install lock per project, so two repos on one host never share it.
const configName = () => {
  try {
    return loadConfig().name || "harness";
  } catch {
    return "harness";
  }
};
...
const lock = `/tmp/${configName()}-playwright-mcp-install.lock`;
