...
const WHITELIST = [
  ...
  /^wc -l( |$)/,
  /^git -C [^ ]+ add MEMORY\.md *$/,
  // Author identity is pinned.
  /^git -C [^ ]+ -c user\.name=['"]?Documentator['"]? -c user\.email=['"]?documentator@harness\.local['"]? commit -m /,
  /^git -C [^ ]+ -c user\.email=['"]?documentator@harness\.local['"]? -c user\.name=['"]?Documentator['"]? commit -m /,
];
...
runStandalone(import.meta.url, "restrict-documentator-bash", check);
