...
- **e2e suite**: you do NOT launch it, and `bash-guard` refuses the command if you try. The `e2e-on-feature-review` SubagentStop hook runs it for you, on the integrated `_session` worktree, and ONLY when the feature review above APPROVED (it parses the reviewer's contract line itself, with the same parser that writes the `FEATURE-quality-reviewer` flag), so a `BLOCKED` review never pays for the suite. **A missing flag is never a reason to re-run a feature review**: the suite gates on the verdict, not on the flag, so re-reviewing to produce one buys nothing and costs a full opus pass. It uses an ISOLATED, slot-leased Supabase instance and tears it down. Read the outcome from `<session_dir>/e2e-result.json` (`status`: `passed` | `skipped` | `failed`, plus the last 40 output lines) after the reviewer returns; the same outcome is appended to `harness-progress.log`. A missing file means the hook did not run the suite (no session worktree, or the review did not approve).
