...
// sits under .claude/, but here it IS the repo.
...
describe("config loader", () => {
  ...
  // This repo's own config, which doubles as the reference a consuming project copies.
  // `deploy` is absent on purpose: the harness itself has no database to migrate, and a
  // capability exists iff its block is present.
  test("the committed harness.config.json is valid", () => {
    clearConfigCache();
    const cfg = loadConfig(REPO_ROOT);
    expect(cfg.name).toBe("aiharness");
    expect(isDeployEnabled(cfg)).toBe(false);
